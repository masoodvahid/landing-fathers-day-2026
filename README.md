...
| **Discount tiers** | `<section id="tiers">` (search `EDIT THESE TIERS`) | ⚠️ Currently a sensible "spend more, save more" default (1→20%, 2→30%, 3→40%, 4+→50%). Replace with the exact rules from your discount-rule image. |
...
## Outstanding items
- [ ] Confirm the **exact discount tiers** (the attached rule image didn't transfer into the build environment).
